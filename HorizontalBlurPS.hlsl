...
float4 main(float4 color : COLOR0, float2 texCoord : TEXCOORD0) : SV_Target0
{
	...
	/*
	float2 tc1 = {texCoord[0]-pixelSize * 4, texCoord[1]};
	float2 tc2 = {texCoord[0]-pixelSize * 3, texCoord[1]};
	float2 tc3 = {texCoord[0]-pixelSize * 2, texCoord[1]};
	float2 tc4 = {texCoord[0]-pixelSize, texCoord[1]};
	float2 tc5 = texCoord;
	float2 tc6 = {texCoord[0]+pixelSize, texCoord[1]};
	float2 tc7 = {texCoord[0]+pixelSize * 2, texCoord[1]};
	float2 tc8 = {texCoord[0]+pixelSize * 3, texCoord[1]};
	float2 tc9 = {texCoord[0]+pixelSize * 4, texCoord[1]};
	*/
...
float4 main(VertexToPixel input) : SV_TARGET
{
	...
	float2 tc1 = {input.texcoord[0]-blurSize, input.texcoord[1]};
	float2 tc2 = {input.texcoord[0], input.texcoord[1]};
	float2 tc3 = {input.texcoord[0]+blurSize, input.texcoord[1]};
	...
	float2 tc7 = {input.texcoord[0]-blurSize, input.texcoord[1]};
	float2 tc8 = {input.texcoord[0], input.texcoord[1]};
	float2 tc9 = {input.texcoord[0]+blurSize, input.texcoord[1]};
